...
{
    public class MofumofuEffect : MonoBehaviour
    {
        [Tooltip("The Universal Renderer Data being used.")]
        ...
        {
            //Editor下生成physTex
            if (settings.physTex == null)
            {
                Texture2D velTex = new(1, 1, TextureFormat.RGBA64, false);
                velTex.SetPixel(0, 0, PackNormal(Vector3.zero));
                velTex.Apply();
                settings.physTex = velTex;
            }
        ...
        public void CreateFeature()
        {
            ...
            if (SortingLayer.layers.Length <= settings.sortingLayerIndex || settings.sortingLayerIndex < 0)
            {
                Debug.LogError("Sorting Layer Index is out of range! " +
                    "The property in Mofumofu Effect Component'settings shouldn't smaller than 0 or larger than the count of sorting layers. " +
                    "And, tags are not included in packages. Create some your own.");
                return;
            }
